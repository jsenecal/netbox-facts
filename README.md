...
- **Device page integration**: A "Facts" tab on every collected device shows its pending entries, the most recent collection per collector type, and the enabled plans whose scope covers it; a "Pending Facts Changes" dashboard widget puts the review backlog on the NetBox home page
...
- **GraphQL API**: MAC addresses, MAC vendors, collection plans, facts reports, and report entries are queryable from NetBox's GraphQL schema
